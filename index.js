...
const { willmenu } = require('./src/willmenu')
...
const vcard = 'BEGIN:VCARD\n' // Will Mods Domina
            ...
            + 'FN:Will\n' // Wilian Gualbino 
            + 'ORG:Owner Bot;\n' // Tio Will @putswill
            + 'TEL;type=CELL;type=VOICE;waid=5516991541171:+55 (16) 99154-1171\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `OIE GOSTOSO(A) @${num.split('@')[0]}\n{ BEM VINDO(A) AO GRUPO } [ *${mdata.subject}* ] \n__________________________\nAPRESENTE-SE!! \nNOME : \nFOTO : \nIDADE : \nCIDADE : \nUSE *HELP PARA CHAMAR O BOT \nPARA SER UM(A) ADMINISTRADOR(A) DO GRUPO TEM QUE MAMAR O TIO WILL \n__________________________\nFE NO PAI QUE O INIMIGO CAI!!`
				...
				teks = `❬ XAU RASCUNHO DO CAPETA, NAO ERA NEM BEM VINDO(A) MESMO... @${num.split('@')[0]} PARA VOLTAR VAI TER QUE MAMAR O TIO WILL OU PAGAR UMA COCA-COLA ❭`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛Calma sou um bot nao seu escravo⌛',
				success: '✔️ Sucesso ✔️',
			...
			const ownerNumber = ["5516991541171@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5516991541171@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				break
				case 'willmenu':
					client.sendMessage(from, willmenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `❬ Pessoal, essa praga aqui chamada acabou de perder seu cargo de admin, digitem (f) no chat ksks ❭ :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`❬ Pessoal, essa praga aqui chamada @${mentioned[0].split('@')[0]} acabou de perder seu cargo de admin, digitem (f) no chat ksks ❭`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'hentai':
                  ...
                  case 'promover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `❬ Pessoal, esse membro comum acaba de virar um escravo do Tio Will, respeitem o(a) novo(a) administrador(a) ok? ❭ :\n`
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`❬ Pessoal, esse membro comum @${mentioned[0].split('@')[0]} acaba de virar um escravo do Tio Will, respeitem o(a) novo(a) administrador(a) ok? ❭`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, '➸ Meu Criador: *>>Tio Will Lindo<<*.',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite Will|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo Will BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall 551699154...`)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = '❬ Contatos bloqueados pelo bot do Tio Will ❭ :\n'
                   ...
                   case 'chatlist':
					client.updatePresence(from, Presence.composing)  
					teks = 'Essa e a lista de todos os numeros do chat :\n'
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 Will BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'marcar4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Transmissão criada com sucesso')
					}
					break
					case 'tmctt':
					client.updatePresence(from, Presence.composing) 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Grupo de transmissão criado sucesso')
					}
					break
				case 'alay':
                    client.updatePresence(from, Presence.composing) 
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `❬ Numero do criador do grupo e do bot ❭ : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'remover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '❬ REMOVI MESMO CARALHO, ALEM DE SER GHOST AINDA JOGA FREE FIRE E OUVE K-POP KSKS ❭  :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`❬ REMOVI MESMO CARALHO @${mentioned[0].split('@')[0]} ALEM DE SER GHOST AINDA JOGA FREE FIRE E OUVE K-POP KSKS ❭`, mentioned, true)
						client.groupRemove(from, mentioned)
	    ...
	    case 'kapankah':
			...
			case 'fechargp':
               ...
               client.updatePresence(from, Presence.composing) 
                ...
                hasil = `*Aqui está seu link encurtado:* ${data.result}`
                reply(hasil)
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Musica encontrada!!*\nJudul : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE ESTOU ENVIANDO, POR FAVOR NAO FLOODAR*`
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf Will|BOT`)
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo |Will|BOT`)
					reply(mess.wait)
            ...
            break
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo WillBOT`)
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo2 WillBOT`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo Will|BOT`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}burnpaper WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}lovepaper WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodblock WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}qowheart WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}mutgrass WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}undergocean WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodenboards WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}wolfmetal WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}metalictglow WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				case 'clone':
					...
					try {
						...
						mentions(`Foto de perfil clonada com sucesso, usando a foto de @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Falhou, talvez esteja sem foto ou foto apenas para contato')
					}
					break
                                /*case 'magernulis':
                                        var gh = body.slice(11)
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}pubglogo Will|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter WillBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
